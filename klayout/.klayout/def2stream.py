# v1 Zerun Fri 29 May 11:43:27 CEST 2026
# - download this from https://github.com/The-OpenROAD-Project/OpenROAD-flow-scripts/blob/7209991acc4f95b3bcf90b97e6fbbf98eb5c4f67/flow/util/def2stream.py
# - resolve the issue with relative path to lef/def defined in lyt file
# - change the allow_empty=allow_empty
# - add lef_files option
try:
    import pya
except ImportError:
    pya = None
...
def merge_gds(
    ...
    Args:
        ...
        lef_files: to give lef files also to the script #Zerun 
    ...
    ########
    # Zerun, add this part to solve the path issue   
    # ---
    cfg = layout_options.lefdef_config
 
    # Paths in .lyt are relative to CWD — make them absolute before
    # KLayout re-anchors them against the DEF file's directory
    if cfg.map_file:
        cfg.map_file = os.path.abspath(cfg.map_file)
    
    if cfg.lef_files:
        cfg.lef_files = [os.path.abspath(f) for f in cfg.lef_files]

    # Still allow explicit override via layer_map argument
    if len(layer_map) > 0:
        cfg.map_file = os.path.abspath(layer_map)

    # Append extra LEF files if explicitly passed (don't replace, extend)
    if len(lef_files) > 0:
        extra = [os.path.abspath(f) for f in lef_files.split()]
        cfg.lef_files = list(cfg.lef_files or []) + extra
    # ---end--- #
