...
#include "../user_lib/inc/aes.h"          // SW AES (KeyExpansion, Cipher)
#include "../user_lib/inc/aes_en_acc.h"   // HW AES Encryption Accelerator
#include "../user_lib/inc/aes_vector.h"   // AES test vectors (uint32_t words)
...
int main() {
    ...
    uart_init();
    printf("--- AES-128 Encryption Testing ---\n\n");
    uart_write_flush();

    // --- Prepare SW vectors from shared HW vectors ---

    // aes_en[0] contains: Input = plaintext, Output = ciphertext
    const aes_vector_t *vec = &aes_en[0];
    ...
    aes_en_acc((uint32_t*)vec->Input, (uint32_t*)vec->Key, hw_out_words);
